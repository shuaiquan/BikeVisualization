	...
	protected void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
		...
		try{
			...
			if(dataFrom.equals("1")){
				sql = "select leasestation,sum(bikenum) from b_leaseinfohis_sum where leasedate='"+date+"' and leasetime='"+time+"' group by leasestation";
			}else if(dataFrom.equals("2")){
				sql = "select returnstation,sum(bikenum) from b_returninfohis_sum where returndate='"+date+"' and returntime='"+time+"' group by returnstation";
			}
